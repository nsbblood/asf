...
def fuzzy_math(num1,operator,num2):
    ...
    if result <0:
        ...
        return 'small number, I can deal with that'
    elif result< 20:
        return ' A medium sized number'
    else:
        return ' A big number you ...'
...
class TestFuzzymath:
        def test_non_int_input_for_num1(self):
            with pytest.raises(Exception) as exc_info:
                fuzzy_math('hi','+',2)
                assert 'fuzzy math on int' in str(exc_info)
        ...
        def test_addition_with_medium_result(self):
            assert 'medium' in fuzzy_math(9,'+',8)

        def test_addition_with_large_result(self):
            assert 'big' in fuzzy_math(10,'+',15)
